...
pub fn pack_err<S, E>(status: S, err: E) -> warp::reject::Rejection
where
...
pub async fn unpack_err(
    rejection: warp::reject::Rejection,
) -> Result<impl warp::Reply, std::convert::Infallible> {
    let reply = if rejection.is_not_found() {
        let problem = HttpApiProblem::with_title_and_type_from_status(http::StatusCode::NOT_FOUND);
        ...
        let problem =
            HttpApiProblem::with_title_and_type_from_status(http::StatusCode::METHOD_NOT_ALLOWED);
        reply_from_problem(&problem)
    } else {
